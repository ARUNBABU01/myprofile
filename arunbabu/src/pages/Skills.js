...
export default function Skills(){
    ...
    skillSets.sort( (a ,b) =>  myprofile.skills[a].length - myprofile.skills[b].length)
    ...
    return (
        <>
            ...
            <Typography style={{fontFamily:"sans-serif", marginLeft:"30px"}} variant="h4" display={"flex"} color="info.main">What I have?</Typography>
<br /><br /><br />
            ...
            return (
                <Grid container key={'skill'+ type} display="flex">
                    ...
                    //  myprofile.skills[type].length > 10 ? null : 
                     <Typography variant='h6' component={'div'}>{type_clean}</Typography>}
                    <CSSTransition key={'skill'+ type + 'trans'} in={true} classNames={classes.slideInMUI} timeout={1000 + (i * 1000)}>
                    {
                        
                    <Grid container item direction={'row'} spacing={1} 
                    style={{width:'100%', alignItems:'center', justifyContent:'flex-start', }}>
                        { myprofile.skills[type].length > 10 ?
                        <Grid container item key={'skill'+ type} width="100%" display={"flex"}>
                        <>
                            <SkillTableMUI
